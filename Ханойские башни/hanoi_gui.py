...
class HanoiSolver:
    ...
    def __init__(self, master):
        ...
        Label(self.toolbar, text = "Задать кол-во дисков: ").pack(side = LEFT)
        ...
        Button(self.toolbar, text = 'Сначало', command = self.generate).pack(side = RIGHT)
    ...
    def check_num_disks(self):
        try:
            ...
            if num > self.max_disks:
                messagebox.showwarning("Предупреждение", f"Максимум {self.max_disks} дисков")
                return False
            else:
                return True
        except ValueError:
